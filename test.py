...
def erode_cv(img, kernel):
    return cv2.erode(img, kernel, iterations=1)

def dilate_cv(img, kernel):
    return cv2.dilate(img, kernel, iterations=1)

def median_cv(img, kernel_size):
    return cv2.medianBlur(img, kernel_size)

def gaussian_cv(img, kernel_size):
    return cv2.GaussianBlur(img, (kernel_size, kernel_size), 1)

def lighting_correction_cv(img):
...
#Gaussian filter
def gaussian(img, kernel, sigma = 1):
    img_h, img_w = img.shape
...
filtered_img = show_all_img(img, 'median')
